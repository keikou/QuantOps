...
PLAN_DOC = REPO_ROOT / "docs" / "Alpha_strategy_selection_intelligence_packet05_effective_selection_slate_plan.md"
V12_API_ROOT = REPO_ROOT / "apps" / "v12-api"
TEST_RUNTIME_DIR = Path(tempfile.mkdtemp(prefix="verify-assi05-", dir=str(REPO_ROOT / "runtime")))
...
def main() -> None:
    ...
    if not PLAN_DOC.exists():
        failures.append("missing_assi05_plan_doc")
    else:
        text = PLAN_DOC.read_text(encoding="utf-8")
        for needle in [
            "/alpha/intelligence/effective-selection-slate/latest",
            "effective_status",
            "selected_for_promotion_review",
            "selected_for_shadow_review",
            "returned_to_research",
            "deferred",
        ]:
    ...
    alpha_service.store.append(
        "alpha_registry",
        {
            ...
            "state": "candidate",
            "source": "assi05_test",
            "notes": "selection slate peer",
        },
    )
    alpha_service.store.append(
        "alpha_rankings",
        {
            "ranking_id": "assi05-ranking-peer",
    ...
    _seed_run(
        CONTAINER,
        now=now,
        run_id="run-assi05-prev",
        cycle_id="cycle-assi05-prev",
    ...
    _seed_run(
        ...
        run_id="run-assi05-next",
        cycle_id="cycle-assi05-next",
    ...
    payload = service.effective_selection_slate_latest(limit=20)

    if payload.get("status") != "ok":
        failures.append("payload_status_not_ok")
    if payload.get("run_id") != "run-assi05-next":
    ...
    statuses = {str(item.get("effective_status") or "") for item in items}
    if "selected_for_shadow_review" not in statuses:
        failures.append("shadow_status_missing")
    if "returned_to_research" not in statuses:
        failures.append("research_status_missing")
    if "deferred" not in statuses:
        failures.append("deferred_status_missing")

    for item in items:
        queue_name = str(item.get("arbitrated_queue_name") or "")
        effective_status = str(item.get("effective_status") or "")
        if queue_name == "promotion_candidate" and effective_status != "selected_for_promotion_review":
            failures.append("promotion_mapping_invalid")
        if queue_name == "shadow_review" and effective_status != "selected_for_shadow_review":
            failures.append("shadow_mapping_invalid")
        if queue_name == "research_return" and effective_status != "returned_to_research":
            failures.append("research_mapping_invalid")
        if queue_name == "deferred_watchlist" and effective_status != "deferred":
            failures.append("deferred_mapping_invalid")

    summary = payload.get("decision_summary") or {}
    if int(summary.get("selected_for_shadow_review", 0) or 0) < 1:
        failures.append("selected_for_shadow_review_count_invalid")
    if int(summary.get("returned_to_research", 0) or 0) < 1:
        failures.append("returned_to_research_count_invalid")
    if int(summary.get("deferred", 0) or 0) < 1:
        failures.append("deferred_count_invalid")
