...
PLAN_DOC = REPO_ROOT / "docs" / "Research_promotion_intelligence_packet01_plan.md"
V12_API_ROOT = REPO_ROOT / "apps" / "v12-api"
TEST_RUNTIME_DIR = Path(tempfile.mkdtemp(prefix="verify-rpi01-", dir=str(REPO_ROOT / "runtime")))
...
def main() -> None:
    ...
    if not PLAN_DOC.exists():
        failures.append("missing_rpi01_plan_doc")
    else:
        text = PLAN_DOC.read_text(encoding="utf-8")
        for needle in [
            "/research-factory/intelligence/promotion-agenda/latest",
            "promotion_action",
            "promote / advance / stay_queued / demote / retire",
        ]:
    ...
    alpha_service.store.append(
        "alpha_registry",
        {
            ...
            "state": "candidate",
            "source": "rpi01_test",
            "notes": "promotion agenda peer",
        },
    )
    alpha_service.store.append(
        "alpha_rankings",
        {
            "ranking_id": "rpi01-ranking-peer",
    ...
    _seed_run(
        CONTAINER,
        now=now,
        run_id="run-rpi01-prev",
        cycle_id="cycle-rpi01-prev",
    ...
    _seed_run(
        ...
        run_id="run-rpi01-next",
        cycle_id="cycle-rpi01-next",
    ...
    payload = service.promotion_agenda_latest(limit=20)

    if payload.get("status") != "ok":
        failures.append("payload_status_not_ok")
    if payload.get("run_id") != "run-rpi01-next":
        failures.append("run_id_mismatch")

    by_alpha = {str(item.get("alpha_id") or ""): item for item in list(payload.get("items") or [])}
    if by_alpha.get("mean_reversion_alpha", {}).get("promotion_action") != "advance":
        failures.append("mean_reversion_should_advance")
    if by_alpha.get("mean_reversion_peer_alpha", {}).get("promotion_action") != "stay_queued":
        failures.append("peer_should_stay_queued")
    if by_alpha.get("promoted_weak_alpha", {}).get("promotion_action") != "retire":
        failures.append("promoted_weak_should_retire")
    if by_alpha.get("breakout_alpha", {}).get("promotion_action") != "retire":
        failures.append("breakout_should_retire")

    summary = payload.get("decision_summary") or {}
    if int(summary.get("advance_actions", 0) or 0) < 1:
        failures.append("advance_actions_invalid")
    if int(summary.get("stay_queued_actions", 0) or 0) < 1:
        failures.append("stay_queued_actions_invalid")
    if int(summary.get("retire_actions", 0) or 0) < 1:
        failures.append("retire_actions_invalid")
