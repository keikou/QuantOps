...
PLAN_DOC = REPO_ROOT / "docs" / "Alpha_strategy_selection_intelligence_packet02_strategy_action_plan.md"
V12_API_ROOT = REPO_ROOT / "apps" / "v12-api"
TEST_RUNTIME_DIR = Path(tempfile.mkdtemp(prefix="verify-assi02-", dir=str(REPO_ROOT / "runtime")))
...
def main() -> None:
    ...
    _seed_run(
        CONTAINER,
        now=now,
        run_id="run-assi02-prev",
        cycle_id="cycle-assi02-prev",
    ...
    _seed_run(
        ...
        run_id="run-assi02-next",
        cycle_id="cycle-assi02-next",
    ...
    payload = service.strategy_action_latest(limit=10)

    if payload.get("status") != "ok":
        failures.append("payload_status_not_ok")
    if payload.get("run_id") != "run-assi02-next":
    ...
    for item in [btc_item, eth_item, sol_item]:
        if item is None:
            failures.append("missing_family_item")
            continue
        if str(item.get("resolved_strategy_action") or "") not in {"promote", "shadow", "research", "defer"}:
            failures.append("invalid_resolved_strategy_action")
        if str(item.get("review_priority") or "") not in {"immediate", "high", "medium", "deferred"}:
            failures.append("invalid_review_priority")

    if eth_item and eth_item.get("resolved_strategy_action") != "shadow":
        failures.append("eth_should_shadow")
    if eth_item and eth_item.get("strategy_action_rationale") != "selection_viable_but_not_promotion_ready":
        failures.append("eth_rationale_invalid")

    if btc_item and btc_item.get("resolved_strategy_action") != "research":
        failures.append("btc_should_research")
    if btc_item and btc_item.get("strategy_action_rationale") != "selection_deprioritized_or_weak":
        failures.append("btc_rationale_invalid")

    if sol_item and sol_item.get("resolved_strategy_action") != "research":
        failures.append("sol_should_research")

    summary = payload.get("decision_summary") or {}
    if int(summary.get("shadow_actions", 0) or 0) < 1:
        failures.append("shadow_actions_missing")
    if int(summary.get("research_actions", 0) or 0) < 1:
        failures.append("research_actions_missing")
    if int(summary.get("defer_actions", 0) or 0) < 1:
        failures.append("defer_actions_missing")
