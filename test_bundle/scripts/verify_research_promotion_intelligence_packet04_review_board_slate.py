...
PLAN_DOC = REPO_ROOT / "docs" / "Research_promotion_intelligence_packet04_review_board_slate_plan.md"
V12_API_ROOT = REPO_ROOT / "apps" / "v12-api"
TEST_RUNTIME_DIR = Path(tempfile.mkdtemp(prefix="verify-rpi04-", dir=str(REPO_ROOT / "runtime")))
...
def main() -> None:
    ...
    if not PLAN_DOC.exists():
        failures.append("missing_rpi04_plan_doc")
    else:
        text = PLAN_DOC.read_text(encoding="utf-8")
        for needle in [
            "/research-factory/intelligence/review-board-slate/latest",
            "board_decision",
            "board_rationale",
            "board_summary",
        ]:
    ...
    alpha_service.store.append(
        "alpha_registry",
        {
            ...
            "state": "candidate",
            "source": "rpi04_test",
            "notes": "review board peer",
        },
    )
    alpha_service.store.append(
        "alpha_rankings",
        {
            "ranking_id": "rpi04-ranking-peer",
    ...
    _seed_run(
        CONTAINER,
        now=now,
        run_id="run-rpi04-prev",
        cycle_id="cycle-rpi04-prev",
    ...
    _seed_run(
        ...
        run_id="run-rpi04-next",
        cycle_id="cycle-rpi04-next",
    ...
    store.append(
        ...
        [
            ...
            {
                "experiment_id": "exp-rpi04-breakout",
                "created_at": created_at,
    ...
    payload = service.review_board_slate_latest(limit=20)

    if payload.get("status") != "ok":
        failures.append("payload_status_not_ok")
    if payload.get("run_id") != "run-rpi04-next":
        failures.append("run_id_mismatch")

    by_alpha = {str(item.get("alpha_id") or ""): item for item in list(payload.get("items") or [])}

    if by_alpha.get("mean_reversion_alpha", {}).get("board_decision") != "approve_shadow_review":
        failures.append("mean_reversion_should_approve_shadow_review")
    if by_alpha.get("breakout_alpha", {}).get("board_decision") != "approve_retirement_review":
        failures.append("breakout_should_approve_retirement_review")
    if by_alpha.get("trend_alpha", {}).get("board_decision") != "hold":
        failures.append("trend_should_hold")
    if by_alpha.get("mean_reversion_peer_alpha", {}).get("board_decision") != "hold":
        failures.append("peer_should_hold")

    summary = payload.get("board_summary") or {}
    if int(summary.get("approve_shadow_review_count", 0) or 0) < 1:
        failures.append("approve_shadow_review_count_invalid")
    if int(summary.get("approve_retirement_review_count", 0) or 0) < 1:
        failures.append("approve_retirement_review_count_invalid")
    if int(summary.get("hold_count", 0) or 0) < 1:
        failures.append("hold_count_invalid")
