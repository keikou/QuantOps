...
PLAN_DOC = REPO_ROOT / "docs" / "Research_promotion_intelligence_packet05_deterministic_review_outcome_resolution_plan.md"
V12_API_ROOT = REPO_ROOT / "apps" / "v12-api"
TEST_RUNTIME_DIR = Path(tempfile.mkdtemp(prefix="verify-rpi05-", dir=str(REPO_ROOT / "runtime")))
...
def main() -> None:
    ...
    if not PLAN_DOC.exists():
        failures.append("missing_rpi05_plan_doc")
    else:
        text = PLAN_DOC.read_text(encoding="utf-8")
        for needle in [
            "/research-factory/intelligence/deterministic-review-outcomes/latest",
            "resolved_review_outcome",
            "outcome_reason_code",
            "downstream_target_state",
        ]:
    ...
    alpha_service.store.append(
        "alpha_registry",
        {
            ...
            "state": "candidate",
            "source": "rpi05_test",
            "notes": "deterministic outcome peer",
        },
    )
    alpha_service.store.append(
        "alpha_rankings",
        {
            "ranking_id": "rpi05-ranking-peer",
    ...
    _seed_run(
        CONTAINER,
        now=now,
        run_id="run-rpi05-prev",
        cycle_id="cycle-rpi05-prev",
    ...
    _seed_run(
        ...
        run_id="run-rpi05-next",
        cycle_id="cycle-rpi05-next",
    ...
    store.append(
        ...
        [
            ...
            {
                "experiment_id": "exp-rpi05-breakout",
                "created_at": created_at,
    ...
    payload = service.deterministic_review_outcomes_latest(limit=20)

    if payload.get("status") != "ok":
        failures.append("payload_status_not_ok")
    if payload.get("run_id") != "run-rpi05-next":
        failures.append("run_id_mismatch")

    by_alpha = {str(item.get("alpha_id") or ""): item for item in list(payload.get("items") or [])}

    if by_alpha.get("mean_reversion_alpha", {}).get("resolved_review_outcome") != "shadow":
        failures.append("mean_reversion_should_shadow")
    if by_alpha.get("mean_reversion_alpha", {}).get("downstream_target_state") != "shadow":
        failures.append("mean_reversion_target_state_invalid")
    if by_alpha.get("breakout_alpha", {}).get("resolved_review_outcome") != "retire":
        failures.append("breakout_should_retire")
    if by_alpha.get("breakout_alpha", {}).get("downstream_target_state") != "retired":
        failures.append("breakout_target_state_invalid")
    if by_alpha.get("trend_alpha", {}).get("resolved_review_outcome") != "needs_more_evidence":
        failures.append("trend_should_need_more_evidence")
    if by_alpha.get("trend_alpha", {}).get("previous_review_status") != "research_backlog":
        failures.append("trend_previous_review_status_invalid")

    summary = payload.get("outcome_summary") or {}
    if int(summary.get("shadow_count", 0) or 0) < 1:
        failures.append("shadow_count_invalid")
    if int(summary.get("retire_count", 0) or 0) < 1:
        failures.append("retire_count_invalid")
    if int(summary.get("needs_more_evidence_count", 0) or 0) < 1:
        failures.append("needs_more_evidence_count_invalid")
