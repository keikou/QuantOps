...
PLAN_DOC = REPO_ROOT / "docs" / "Alpha_strategy_selection_intelligence_packet04_family_budget_arbitration_plan.md"
V12_API_ROOT = REPO_ROOT / "apps" / "v12-api"
TEST_RUNTIME_DIR = Path(tempfile.mkdtemp(prefix="verify-assi04-", dir=str(REPO_ROOT / "runtime")))
...
def main() -> None:
    ...
    if not PLAN_DOC.exists():
        failures.append("missing_assi04_plan_doc")
    else:
    ...
    # Force two momentum candidates into the same shadow queue.
    alpha_service.store.append(
        "alpha_registry",
        {
            ...
            "feature_dependencies_json": alpha_service.store.to_json(["mom_1"]),
            "risk_profile": "balanced",
            "execution_sensitivity": 0.18,
            "state": "candidate",
            "source": "assi04_test",
            "notes": "family budget peer",
        },
    )
    alpha_service.store.append(
        "alpha_rankings",
        {
            "ranking_id": "assi04-ranking-peer",
    ...
    _seed_run(
        CONTAINER,
        now=now,
        run_id="run-assi04-prev",
        cycle_id="cycle-assi04-prev",
    ...
    _seed_run(
        ...
        run_id="run-assi04-next",
        cycle_id="cycle-assi04-next",
    ...
    payload = service.family_budget_arbitration_latest(limit=20)

    if payload.get("status") != "ok":
        failures.append("payload_status_not_ok")
    if payload.get("run_id") != "run-assi04-next":
        failures.append("run_id_mismatch")

    shadow_rows = [
        item for item in list(payload.get("items") or [])
        if str(item.get("alpha_family") or "") == "mean_reversion"
        and str(item.get("queue_name") or "") == "shadow_review"
    ]
    if len(shadow_rows) < 2:
        failures.append("not_enough_shadow_rows")
    winners = [item for item in shadow_rows if bool(item.get("winning_in_family"))]
    deferred = [item for item in shadow_rows if str(item.get("family_budget_state") or "") == "deferred_by_family_budget"]
    if len(winners) != 1:
        failures.append("family_budget_winner_count_invalid")
    if len(deferred) < 1:
        failures.append("family_budget_deferral_missing")
    for item in deferred:
        if item.get("arbitrated_queue_name") != "research_return":
            failures.append("deferred_queue_invalid")

    summary = payload.get("decision_summary") or {}
    if int(summary.get("family_budget_deferrals", 0) or 0) < 1:
        failures.append("family_budget_deferrals_invalid")
