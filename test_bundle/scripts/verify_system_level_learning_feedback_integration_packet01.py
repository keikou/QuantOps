...
PLAN_DOC = REPO_ROOT / "docs" / "System_level_learning_feedback_integration_packet01_plan.md"
V12_API_ROOT = REPO_ROOT / "apps" / "v12-api"
TEST_RUNTIME_DIR = Path(tempfile.mkdtemp(prefix="verify-sllfi01-", dir=str(REPO_ROOT / "runtime")))
...
def main() -> None:
    ...
    if not PLAN_DOC.exists():
        failures.append("missing_sllfi01_plan_doc")
    else:
    ...
    families = ["momentum", "mean_reversion", "event"]
    ...
    alpha_service.store.append(
        "alpha_rankings",
        {
            "ranking_id": "sllfi01-ranking-peer",
    ...
    alpha_service.store.append(
        "alpha_rankings",
        {
            "ranking_id": "sllfi01-ranking-weak",
            "created_at": (now - timedelta(minutes=9)).isoformat(),
            "alpha_id": "promoted_weak_alpha",
            "rank_score": 0.29,
    ...
    _seed_run(
        CONTAINER,
        now=now,
        run_id="run-sllfi-prev",
        cycle_id="cycle-sllfi-prev",
    ...
    store.append(
        ...
        [
            ...
            {
                "experiment_id": "exp-sllfi-breakout",
                "created_at": created_at,
    ...
    rpi = ResearchPromotionIntelligenceService()
    rpi.persisted_governed_state_transitions_latest(limit=20)
    ...
    if payload.get("run_id") != "run-sllfi-next":
    ...
    if mean_reversion.get("learning_action") != "reinforce":
        failures.append("mean_reversion_should_reinforce")
    if event_family.get("learning_action") != "caution":
        failures.append("event_should_caution")

    mean_transitions = (mean_reversion.get("governed_transition_feedback") or {})
    if int(mean_transitions.get("shadow_states", 0) or 0) < 1:
        failures.append("mean_reversion_shadow_transition_missing")
    event_transitions = (event_family.get("governed_transition_feedback") or {})
    if int(event_transitions.get("retired_states", 0) or 0) < 1:
        failures.append("event_retired_transition_missing")

    summary = payload.get("feedback_summary") or {}
    if int(summary.get("reinforce_families", 0) or 0) < 1:
        failures.append("reinforce_count_invalid")
    if int(summary.get("caution_families", 0) or 0) < 1:
        failures.append("caution_count_invalid")
    if str(summary.get("system_learning_action") or "") not in {
        "reinforce_positive_families",
        "contain_negative_feedback",
        "rebalance_mixed_feedback",
        "stabilize_before_expansion",
        "observe_and_collect_more_feedback",
    }:
        failures.append("system_learning_action_invalid")
