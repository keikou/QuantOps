...
PLAN_DOC = REPO_ROOT / "docs" / "Research_promotion_intelligence_packet06_persisted_governed_state_transition_plan.md"
V12_API_ROOT = REPO_ROOT / "apps" / "v12-api"
TEST_RUNTIME_DIR = Path(tempfile.mkdtemp(prefix="verify-rpi06-", dir=str(REPO_ROOT / "runtime")))
...
def main() -> None:
    ...
    if not PLAN_DOC.exists():
        failures.append("missing_rpi06_plan_doc")
    else:
        text = PLAN_DOC.read_text(encoding="utf-8")
        for needle in [
            "/research-factory/intelligence/persisted-governed-state-transitions/latest",
            "prior_governed_state",
            "new_governed_state",
            "transition_source_packet",
        ]:
    ...
    alpha_service.store.append(
        "alpha_registry",
        {
            ...
            "state": "candidate",
            "source": "rpi06_test",
            "notes": "persisted transition peer",
        },
    )
    alpha_service.store.append(
        "alpha_rankings",
        {
            "ranking_id": "rpi06-ranking-peer",
    ...
    _seed_run(
        CONTAINER,
        now=now,
        run_id="run-rpi06-prev",
        cycle_id="cycle-rpi06-prev",
    ...
    _seed_run(
        ...
        run_id="run-rpi06-next",
        cycle_id="cycle-rpi06-next",
    ...
    store.append(
        ...
        [
            ...
            {
                "experiment_id": "exp-rpi06-breakout",
                "created_at": created_at,
    ...
    payload = service.persisted_governed_state_transitions_latest(limit=20)

    if payload.get("status") != "ok":
        failures.append("payload_status_not_ok")
    if payload.get("run_id") != "run-rpi06-next":
        failures.append("run_id_mismatch")

    by_alpha = {str(item.get("alpha_id") or ""): item for item in list(payload.get("items") or [])}

    if by_alpha.get("mean_reversion_alpha", {}).get("new_governed_state") != "shadow":
        failures.append("mean_reversion_should_persist_shadow")
    if by_alpha.get("breakout_alpha", {}).get("new_governed_state") != "retired":
        failures.append("breakout_should_persist_retired")
    if by_alpha.get("trend_alpha", {}).get("new_governed_state") != by_alpha.get("trend_alpha", {}).get("prior_governed_state"):
        failures.append("trend_should_not_change_state")
    if by_alpha.get("mean_reversion_alpha", {}).get("transition_source_packet") != "RPI-06":
        failures.append("transition_source_packet_invalid")

    latest_mr = store.fetchone_dict(
        "SELECT state, source FROM alpha_registry WHERE alpha_id=? ORDER BY created_at DESC LIMIT 1",
        ["mean_reversion_alpha"],
    ) or {}
    if latest_mr.get("state") != "shadow":
        failures.append("mean_reversion_latest_state_not_shadow")
    latest_breakout = store.fetchone_dict(
        "SELECT state, source FROM alpha_registry WHERE alpha_id=? ORDER BY created_at DESC LIMIT 1",
        ["breakout_alpha"],
    ) or {}
    if latest_breakout.get("state") != "retired":
        failures.append("breakout_latest_state_not_retired")

    event = store.fetchone_dict(
        "SELECT to_state, event_type FROM alpha_status_events WHERE alpha_id=? ORDER BY created_at DESC LIMIT 1",
        ["mean_reversion_alpha"],
    ) or {}
    if event.get("to_state") != "shadow":
        failures.append("status_event_not_persisted")

    summary = payload.get("transition_summary") or {}
    if int(summary.get("shadow_state_count", 0) or 0) < 1:
        failures.append("shadow_state_count_invalid")
    if int(summary.get("retired_state_count", 0) or 0) < 1:
        failures.append("retired_state_count_invalid")
