...
PLAN_DOC = REPO_ROOT / "docs" / "Research_promotion_intelligence_packet02_candidate_docket_plan.md"
V12_API_ROOT = REPO_ROOT / "apps" / "v12-api"
TEST_RUNTIME_DIR = Path(tempfile.mkdtemp(prefix="verify-rpi02-", dir=str(REPO_ROOT / "runtime")))
...
def main() -> None:
    ...
    if not PLAN_DOC.exists():
        failures.append("missing_rpi02_plan_doc")
    else:
        text = PLAN_DOC.read_text(encoding="utf-8")
        for needle in [
            "/research-factory/intelligence/promotion-candidate-docket/latest",
            "experiment_id",
            "model_id",
            "docket_status",
        ]:
    ...
    alpha_service.store.append(
        "alpha_registry",
        {
            ...
            "state": "candidate",
            "source": "rpi02_test",
            "notes": "promotion docket peer",
        },
    )
    alpha_service.store.append(
        "alpha_rankings",
        {
            "ranking_id": "rpi02-ranking-peer",
    ...
    _seed_run(
        CONTAINER,
        now=now,
        run_id="run-rpi02-prev",
        cycle_id="cycle-rpi02-prev",
    ...
    _seed_run(
        ...
        run_id="run-rpi02-next",
        cycle_id="cycle-rpi02-next",
    ...
    store.append(
        ...
        [
            ...
            {
                "experiment_id": "exp-rpi02-weak",
                "created_at": created_at,
                "dataset_version": "dataset.event.v1",
                "feature_version": "features.event.v1",
                "model_version": "model.event.v1",
                "alpha_id": "promoted_weak_alpha",
                "strategy_id": "event_core",
                "hypothesis": "weak promoted alpha should be retired",
                "hyperparameters_json": store.to_json({"lr": 0.01}),
                "validation_result_json": store.to_json({"walk_forward_sharpe": 0.4}),
                "notes": "seeded by verifier",
                "immutable_record": True,
            },
        ],
    ...
    store.append(
        ...
        [
            ...
            {
                "model_id": "model-rpi02-weak",
                "created_at": created_at,
                "experiment_id": "exp-rpi02-weak",
                "dataset_version": "dataset.event.v1",
                "feature_version": "features.event.v1",
                "model_version": "model.event.v1",
                "validation_metrics_json": store.to_json({"summary_score": 0.28}),
                "state": "deprecated",
                "notes": "seeded by verifier",
            },
        ],
    ...
    payload = service.promotion_candidate_docket_latest(limit=20)

    if payload.get("status") != "ok":
        failures.append("payload_status_not_ok")
    if payload.get("run_id") != "run-rpi02-next":
        failures.append("run_id_mismatch")

    by_alpha = {str(item.get("alpha_id") or ""): item for item in list(payload.get("items") or [])}

    mr_item = by_alpha.get("mean_reversion_alpha", {})
    if mr_item.get("promotion_action") != "advance":
        failures.append("mean_reversion_should_advance")
    if mr_item.get("docket_status") != "shadow_candidate":
        failures.append("mean_reversion_should_be_shadow_candidate")
    if mr_item.get("experiment_id") != "exp-rpi02-mr":
        failures.append("mean_reversion_experiment_missing")
    if mr_item.get("model_id") != "model-rpi02-mr":
        failures.append("mean_reversion_model_missing")
    if mr_item.get("lineage_ready") is not True:
        failures.append("mean_reversion_lineage_not_ready")

    weak_item = by_alpha.get("promoted_weak_alpha", {})
    if weak_item.get("promotion_action") != "retire":
        failures.append("promoted_weak_should_retire")
    if weak_item.get("docket_status") != "retirement_candidate":
        failures.append("promoted_weak_should_be_retirement_candidate")
    if weak_item.get("model_state") != "deprecated":
        failures.append("promoted_weak_model_state_missing")

    breakout_item = by_alpha.get("breakout_alpha", {})
    if breakout_item.get("lineage_ready") is not False:
        failures.append("breakout_should_not_have_lineage")
    if breakout_item.get("docket_status") != "retirement_candidate":
        failures.append("breakout_should_be_retirement_candidate")

    summary = payload.get("decision_summary") or {}
    if int(summary.get("lineage_ready_items", 0) or 0) < 2:
        failures.append("lineage_ready_count_invalid")
    if int(summary.get("shadow_candidate_count", 0) or 0) < 1:
        failures.append("shadow_candidate_count_invalid")
    if int(summary.get("retirement_candidate_count", 0) or 0) < 1:
        failures.append("retirement_candidate_count_invalid")
