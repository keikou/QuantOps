...
class TradingService:
    ...
    def run_once(self) -> dict:
        ...
        if getattr(self.collector, 'fetch_best_bid_ask_many', None):
            try:
                quote_rows = self.collector.fetch_best_bid_ask_many(self.settings.symbols)
                live_quotes = {quote.symbol: quote.to_dict() for quote in quote_rows}
            except Exception:
                live_quotes = {}
        if live_quotes:
            latest_prices = {
                symbol: live_quotes.get(symbol, {"mark_price": float(latest_prices.get(symbol, 0.0) or 0.0), "source": "frame_close_fallback"})
                for symbol in self.settings.symbols
            }
