...
class TruthEngine:
    ...
    def compute_equity_snapshot(self, positions: list[dict[str, Any]], as_of: str) -> dict[str, Any]:
        started_at = time.perf_counter()
        cash_balance = self.latest_cash_balance()

        previous = self.store.fetchone_dict(
            """
            SELECT peak_equity
        ...
        row = compute_equity_snapshot_row(
            cash_balance=cash_balance,
            positions=positions,
            previous_peak_equity=float(previous.get("peak_equity", self.initial_capital) or self.initial_capital),
            reporting_currency="USD",
            as_of=as_of,
        )
        ...
        self.store.append("equity_snapshots", full_row)
